...
public final class ProductService {
    ...
    private static void handle(HttpExchange ex) throws IOException {
        ...
        try { obj = SimpleJson.parseObject(body); }
            ...
            System.err.println("[ProductService] JSON parse error: " + e.getMessage());
            System.err.println("[ProductService] Body: " + body);
            send(ex, 400, "{}"); return;
        }

        String cmd = asString(obj.get("command"));
        if (cmd == null || cmd.isEmpty()) {
            dump(obj, "missing/empty command");
            send(ex, 400, "{}"); return;
        }
        ...
        switch (cmd) {
            ...
            default -> { dump(obj, "unknown command: " + cmd); send(ex, 400, "{}"); }
        }
    }
    ...
    private static void handleCreate(HttpExchange ex, Map<String,Object> obj) throws IOException {
        ...
        if (id == null || name == null || description == null || price == null || quantity == null) {
            dump(obj, "create missing field(s) after casting - id=" + id + " name=" + name + " desc=" + description + " price=" + price + " qty=" + quantity);
            send(ex, 400, "{}"); return;
        }
        if (name.isEmpty() || description.isEmpty()) {
            dump(obj, "create empty name/description");
            send(ex, 400, "{}"); return;
        }
        if (price < 0 || quantity < 0) {
            dump(obj, "create negative price/qty");
            send(ex, 400, "{}"); return;
        }
    ...
    private static void handleUpdate(HttpExchange ex, Map<String,Object> obj) throws IOException {
        Integer id = asInt(obj.get("id"));
        if (id == null) { dump(obj, "update missing id"); send(ex, 400, "{}"); return; }
        ...
        if (obj.containsKey("name")) {
            String name = asString(obj.get("name"));
            if (name == null || name.isEmpty()) { dump(obj, "update bad name"); send(ex, 400, "{}"); return; }
            p.name = name; any = true;
        }
        if (obj.containsKey("description")) {
            String d = asString(obj.get("description"));
            if (d == null || d.isEmpty()) { dump(obj, "update bad description"); send(ex, 400, "{}"); return; }
            p.description = d; any = true;
        }
        if (obj.containsKey("price")) {
            Double price = asDouble(obj.get("price"));
            if (price == null || price < 0) { dump(obj, "update bad price"); send(ex, 400, "{}"); return; }
            p.price = price; any = true;
        }
        if (obj.containsKey("quantity")) {
            Integer q = asIntStrict(obj.get("quantity"));
            if (q == null || q < 0) { dump(obj, "update bad quantity"); send(ex, 400, "{}"); return; }
            p.quantity = q; any = true;
        }

        if (!any) { dump(obj, "update had no fields"); send(ex, 400, "{}"); return; }
        send(ex, 200, SimpleJson.stringify(toMap(p)));
    }
    ...
    private static void handleDelete(HttpExchange ex, Map<String,Object> obj) throws IOException {
        ...
        if (id == null || name == null || price == null || quantity == null) {
            dump(obj, "delete missing field(s)");
            send(ex, 400, "{}"); return;
        }
    ...
    private static void dump(Map<String,Object> obj, String why) {
        System.err.println("[ProductService] 400 because: " + why);
        for (var e : obj.entrySet()) {
            Object v = e.getValue();
            String t = (v == null) ? "null" : v.getClass().getName();
            System.err.println("[ProductService]   " + e.getKey() + " = " + v + " (" + t + ")");
        }
    }
