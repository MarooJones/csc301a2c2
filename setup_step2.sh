...
# Update run script to include jdk.httpserver module on run
perl -0777 -i -pe 's/\(cd "\$OUT\/ISCS" && java /$&--add-modules jdk.httpserver /g; s/\(cd "\$OUT\/UserService" && java /$&--add-modules jdk.httpserver /g; s/\(cd "\$OUT\/ProductService" && java /$&--add-modules jdk.httpserver /g; s/\(cd "\$OUT\/OrderService" && java /$&--add-modules jdk.httpserver /g' runme.sh
...
public final class UserService {
    ...
    private static void handleCreate(HttpExchange ex, Map<String,Object> obj) throws IOException {
        ...
        String ph = Sha256.hexLower(password);
    ...
    private static void handleUpdate(HttpExchange ex, Map<String,Object> obj) throws IOException {
        ...
        if (obj.containsKey("password")) {
            ...
            u.passwordHash = Sha256.hexLower(pw);
            any = true;
        }
        ...
        resp.put("username", u.username);
        resp.put("email", u.email);
        resp.put("password", u.passwordHash);
        send(ex, 200, SimpleJson.stringify(resp));
    }
